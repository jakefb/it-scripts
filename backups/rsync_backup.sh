...
handle_exit() {
  ...
  if [[ $EXIT_CODE -ne 0 && $EXIT_CODE -ne 2 ]]; then
    echo $'\e[31mAn error occurred. Exiting...\e[0m' >&2
  fi
  exit $EXIT_CODE
}
...
if [[ "$DEST_DIR" != *":"* ]]; then
  ...
  if ssh "$REMOTE_ADDRESS" "[[ -d \"$REMOTE_DIR\" ]]"; then
    dest_exists_ask_to_continue
  else
    if (( $? == 255 )); then
      echo "SSH connection failed. Check your connection or SSH configuration." >&2
      exit 1
    fi
  fi
fi
...
if [[ "$SHOULD_CONTINUE" == "y" ]]; then
  ...
  eval "$RSYNC_COMMAND"
  if (( IS_REMOTE_COPY == 1 )); then
    scp "$LOG_PATH" "${DEST_DIR_NO_SLASH}_rsync_log.txt"
  fi
